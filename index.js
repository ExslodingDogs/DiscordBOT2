...
const con = require("./botsettings/prefix.json");
const bot = new Discord.Client({disableEveryone: true});
bot.commands = new Discord.Collection();
const swear = require("./botsettings/swear.js");
const scount = require("./PlayerData/mutedcount.json");
...
bot.on("message", async message => {
  ...
  if(!coins[message.author.id]){
	...
	if(!perms[message.author.id].name == message.author.username){
		perms[message.author.id] ={
			name: message.author.username,
			admin: perms[message.author.id].admin,
			vip: perms[message.author.id].vip,
			default: perms[message.author.id].default,
			clear: perms[message.author.id].clear
		}
		fs.writeFile("./PlayerData/permissions.json", JSON.stringify(perms), (err) => {
			if (err) console.log(err)
		});
	...
	const args = message.content.slice(con.prefix.length).trim().split(/ +/g);
...
bot.on('message', msg => {
	if(msg.guild){
		if(!msg.author.id === "336957269537456138" || !msg.author.id === "377925906494259201"){
		if(!scount[msg.author.id].count){
			scount[msg.author.id] = {
				count: 0
			};
			fs.writeFile("./PlayerData/mutedcount.json", JSON.stringify(scount), (err) =>{
		...
		for (i = 0; i < 554; i++)
		{
			if(lower.indexOf(swear.list[i]) >= 0){
				console.log(i);
				msg.delete();
				if(scount[msg.author.id].count == 3){
					msg.delete();
					let muteRole = client.guilds.get(message.guild.id).roles.find('name', 'muted');
					if(!muteRole) return console.log(`${muterole} not found on ${guild.name}`);
					message.guild.member(user).addRole(muteRole);

				}
				scount[msg.author.id] ={
					count: scount[msg.author.id].count + 1
				};
				fs.writeFile("./PlayerData/mutedcount.json", JSON.stringify(scount), (err) =>{
					if(err) return console.log(err);
				});
